...
async function main() {
  ...
  */
  ...
  let mockFloorPrice;
  mockFloorPrice = hre.ethers.utils.parseUnits('98.5', 18);

  await configurator.connect(admin).setNFTPriceConsumerFloorPrice(nftBaycAddress, mockFloorPrice).then(res=>{
    console.log('succ\n', res);
  }).catch(error=>{
      console.log(ethers.utils.toUtf8String(Object.values(error.body)));
  });
  
  let floorPrice = await nftPriceConsumer.getFloorPrice(nftBaycAddress);
